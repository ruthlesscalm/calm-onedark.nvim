return {
  ...
  config = function()
    ...
    vim.keymap.set({ "n", "i" }, "<M-a>", mark_file, { desc = "Harpoon Add File" })

    -- 2. QUICK MENU (Ctrl + e)
    --    Opens the simple Harpoon menu to drag-and-drop/reorder files.
    vim.keymap.set({ "n", "i" }, "<C-e>", function() harpoon.ui:toggle_quick_menu(harpoon:list()) end,
      { desc = "Harpoon Menu" })

    -- 3. TELESCOPE SEARCH (Leader + h)
    --    Opens the fancy Telescope UI (defined above) to fuzzy search your marked files.
    vim.keymap.set("n", "<leader>t", function() toggle_telescope(harpoon:list()) end, { desc = "Harpoon Telescope" })
